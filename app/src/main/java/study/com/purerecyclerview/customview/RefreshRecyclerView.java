...
public class RefreshRecyclerView extends RecyclerView {
    ...
    public boolean onTouchEvent(MotionEvent e) {
        ...
        if (refreshView.getVisibility() != VISIBLE) return super.onTouchEvent(e);

        LogUtil.i("摁下时候的状态 curState = " + curState);
        switch (e.getAction()) {
            case MotionEvent.ACTION_DOWN:
                curState = STATE_DEFAULT;
                LogUtil.i("LHD  MotionEvent.ACTION_DOWN " + isTop());
...
//                    LogUtil.i("LHD 计算dy2 = " + dy + "  downY = " + downY);
                    ...
                    //为了始终保持recyclerView在最底部，所以需要同步位移recyclerView，来保持bottomView的底部始终在(marginLayoutParams.bottomMargin - refreshViewHeight - 1)的位置
            ...
            case MotionEvent.ACTION_UP:
                //如果不是滑动到底部的抬起动作，则不处理
                LogUtil.i("LHD  MotionEvent.ACTION_UP + " + isTop);
                if (isTop) {
...
//        LogUtil.i("LHD 传递触摸事件");
     ...
     *                   footHeight > refreshViewHeight       的时候是 STATE_RELEASE_TO_LOAD
     * @return 返回当前状态
     */
...
//            LogUtil.i("LHD 设置bottomView的高度 = " + footHeight);
            bottomLp = topView.getLayoutParams();
            bottomLp.height = (int) footHeight;
            topView.setLayoutParams(bottomLp);
        }
